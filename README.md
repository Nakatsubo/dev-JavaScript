...
- <strong>onload</strong> 画像の読み込み後に処理を実行<br>
...
### ウェブカメラ
getUserMedia()メソッドとautoplay属性、video要素のsrcObject属性にウェブカメラのストリームを指定する。<br>
studing now...
...
### SVGをダウンロードするサンプル
outerHTMLプロパティで文字列を取得し、ブラウザごとに適切なデータ形式に変換し、保存する。
...
- <storong>canvas.toDataURL([形式])</strong> メソッド 指定したDataURL形式を取得
...
- <storong>new Blob([配列], オプション)</strong> メソッド Blobオブジェクトを作成

```
HTML
<main class="centering">
  <figure>
    <canvas id="canvas-original" width="150" height="150"></canvas>
    <figcaption>▲ 元の画像</figcaption>
  </figure>
  <figure>
    <canvas id="canvas-effected" width="150" height="150"></canvas>
...
- <strong>setTimeout(関数, ミリ秒)</strong> 一定時間後に処理を実行
...
- <strong>clearTimeout(関数, ミリ秒)</strong> 一定時間後に処理を解除
...
- <strong>Promise.all()</strong> Promiseで並列処理
...
- Promiseで直列処理<br>
外部データを取得してから次の処理につなげる場合などに使用する。
